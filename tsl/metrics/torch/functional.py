...
def rse(
    ...
    if mask is not None:
        err = torch.sqrt(err / _masked_reduce(torch.square(torch.mean(y[mask]) - y[mask]), 'sum', mask, nan_to_zero))
    else:
        err = torch.sqrt(err / _masked_reduce(torch.square(torch.mean(y) - y), 'sum', mask, nan_to_zero))
    return err
